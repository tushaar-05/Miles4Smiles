...
const nstFormRows = parseCSV(nstFormRaw);
const gatewayRows = parseCSV(gatewayRaw);

console.log(`Parsed NST Form Submissions: ${nstFormRows.length}`);
console.log(`Parsed Gateway Transactions: ${gatewayRows.length}`);

// 1. De-duplicate NST Form Submissions (keeping the latest submission per email / URN)
const nstMap = new Map();

for (const row of nstFormRows) {
...
const uniqueNstStudents = Array.from(nstMap.values());
console.log(`Unique NST Students from Form: ${uniqueNstStudents.length}`);

// 2. Map Gateway Transactions with NST Students vs General Public
const matchedNSTInGateway = [];
const generalGatewayRows = [];

// Create quick lookup sets for NST
const nstEmails = new Set(uniqueNstStudents.map(s => s.email.toLowerCase()).filter(Boolean));
const nstUrns = new Set(uniqueNstStudents.map(s => s.urn.toLowerCase()).filter(Boolean));
const nstNames = new Set(uniqueNstStudents.map(s => s.name.toLowerCase().replace(/\s+/g, ' ')));

for (const gw of gatewayRows) {
  const buyerEmail = (gw['Email ID'] || '').toLowerCase().trim();
  const buyerName = (gw['Buyer Name'] || '').trim();
  const buyerNameNorm = buyerName.toLowerCase().replace(/\s+/g, ' ');
  const contactNo = gw['Contact No'] || '';
  const custId = gw['Customer ID'] || '';
  const txnId = gw['Transaction ID'] || '';
  const category = gw['Category'] || 'Competitive 5K';
  const price = parseFloat(gw['Price Per Ticket'] || '0');
  const totalAmt = parseFloat(gw['Total Amount Transferred'] || '0');

  // Check if NST
  const isNst = 
    buyerEmail.endsWith('@adypu.edu.in') ||
    nstEmails.has(buyerEmail) ||
    nstNames.has(buyerNameNorm) ||
    buyerEmail.includes('e26b') || buyerEmail.includes('e25b');

  if (isNst) {
    matchedNSTInGateway.push({
      buyerName,
      buyerEmail,
      contactNo,
      custId,
...
console.log(`Gateway NST Transactions: ${matchedNSTInGateway.length}`);
console.log(`Gateway General Public Transactions: ${generalGatewayRows.length}`);

console.log('\n--- General Public Gateway Transactions ---');
generalGatewayRows.forEach((g, i) => {
  console.log(`[${i+1}] ${g.buyerName} | ${g.buyerEmail} | ${g.contactNo} | ₹${g.totalAmt} | ${g.category} | Cust: ${g.custId} | Txn: ${g.txnId}`);
});
