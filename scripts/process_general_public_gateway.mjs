...
console.log(`Found ${nstInDb.length} NST students in DB.`);
...
for (const line of lines) {
  // Regex parse CSV handling quotes
...
console.log(`Total Gateway Rows: ${gatewayRows.length}`);
...
for (const row of gatewayRows) {
  const isNstEmail = nstEmailSet.has(row.email) || row.email.endsWith('@adypu.edu.in');
  const isNstName = nstNameSet.has(row.buyerName.toLowerCase().trim().replace(/\s+/g, ' '));
...
console.log(`\nFiltered Gateway Summary:`);
console.log(`NST Rows: ${nstRows.length}`);
console.log(`General Public Rows: ${generalRows.length}`);

console.log('\n--- General Public Rows Extracted ---');
generalRows.forEach((g, i) => {
  console.log(`[${i+1}] ${g.buyerName} | ${g.email} | ${g.contactNo} | ₹${g.totalAmt} | ${g.category} | Cust: ${g.custId} | Txn: ${g.txnId}`);
});
