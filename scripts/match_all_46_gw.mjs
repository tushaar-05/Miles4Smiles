...
for (const nst of nstRows) {
  // Find in Gateway by email or name
  let gw = gwRows.find(g => nst.email && g.email === nst.email);
  if (!gw) {
    gw = gwRows.find(g => {
      const gName = g.name.toLowerCase();
      const nName = nst.name.toLowerCase();
      return gName === nName || (nName.split(' ')[0] === gName.split(' ')[0] && nName.split(' ')[1] === gName.split(' ')[1]);
    });
  }

  const amount = gw ? gw.amount : 149; // Default 149 if standard student registration
  const raceType = amount === 99 ? 'Non-Competitive Joy 5K' : 'Competitive 5K';
  
  console.log(`[${nst.index}] ${nst.name} | ${nst.email} | GW Amount: ${gw ? gw.amount : 'Not matched (default 149)'} -> Race: ${raceType} | Proof: ${nst.driveLink ? 'YES' : 'NO'}`);
}
