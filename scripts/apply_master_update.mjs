...
// 1. Process 50 unique NST students from form
...
const uniqueNstList = Array.from(studentMap.values());

// Add the 4 direct gateway NST students who didn't fill form
const studentEmails = new Set(uniqueNstList.map(s => s.email.toLowerCase()).filter(Boolean));
const studentNames = new Set(uniqueNstList.map(s => s.name.toLowerCase().replace(/\s+/g, ' ')));
...
for (const gw of gatewayParsed) {
  ...
  if (isAdypu && !studentEmails.has(email) && !studentNames.has(name.toLowerCase().replace(/\s+/g, ' '))) {
    uniqueNstList.push({
      name,
      email,
    ...
    studentNames.add(name.toLowerCase().replace(/\s+/g, ' '));
  }
}

console.log(`Total Unified NST Students: ${uniqueNstList.length}`);

// Match each NST student against the Gateway transactions
const nstRecordsToDb = [];
let nstPaidCount = 0;
let nstPendingCount = 0;
let nstRevenue = 0;

for (let i = 0; i < uniqueNstList.length; i++) {
  const s = uniqueNstList[i];
  const nameParts = s.name.split(' ');
  const firstName = nameParts[0];
  const lastName = nameParts.slice(1).join(' ') || '';

  // Look for match in Gateway
  const gwMatch = gatewayParsed.find(gw => {
    const gwEmail = gw['Email ID'].toLowerCase().trim();
    const gwName = gw['Buyer Name'].toLowerCase().replace(/\s+/g, ' ');
    const sName = s.name.toLowerCase().replace(/\s+/g, ' ');
    const sEmail = s.email.toLowerCase().trim();
    return (
      (sEmail && gwEmail === sEmail) ||
      (s.phone && gw['Contact No'] && gw['Contact No'].includes(s.phone.slice(-10))) ||
      gwName === sName ||
      (s.urn && gwEmail.includes(s.urn.toLowerCase()))
    );
  });

  const chestNumber = `NST-${101 + i}`;
  const bibNumber = `M4S-NST-${101 + i}`;
  const phone = (gwMatch && gwMatch['Contact No']) || s.phone || '—';

  // Rule: If paid 149 -> Competitive, if paid 99 or 0 -> Non-Competitive Joy
  let paymentStatus = 'pending';
  let amount = 0;
  let raceType = 'Pending Payment';
  let custId = 'unknown';
  let txnId = 'unknown';

  if (gwMatch) {
    paymentStatus = 'paid';
    amount = parseFloat(gwMatch['Total Amount Transferred']) || (gwMatch['Category'].includes('Comp') ? 149 : 99);
    raceType = amount >= 149 ? 'Competitive 5K' : 'Non-Competitive Joy 5K';
    custId = gwMatch['Customer ID'] || 'GATEWAY';
    txnId = gwMatch['Transaction ID'] || 'GATEWAY';
    nstPaidCount++;
    nstRevenue += amount;
  } else if (s.proof && s.proof.startsWith('http')) {
    // Has screenshot proof in form
    paymentStatus = 'paid';
    amount = 149;
  ...
  const urnFormatted = s.urn || (s.email.includes('@adypu.edu.in') ? s.email.split('@')[0].toUpperCase() : '—');
  const city = `NST ADYPU • ${s.study_year || '1st'} Year • URN: ${urnFormatted}`;
  ...
  nstRecordsToDb.push({
    ...
    blood_group: 'O+',
    dob: `${s.study_year || '1st'} Year`,
    weight: s.weight || '—',
    height: s.height || '—',
    t_shirt_size: s.t_shirt_size || 'M',
    email: s.email,
    phone: phone || '—',
    city: city,
    emergency_name: s.proof || '—',
    emergency_phone: '—',
...
// 2. Identify all General Public Gateway Payments
const allNstEmailSet = new Set(uniqueNstList.map(s => s.email.toLowerCase()));
const allNstNameSet = new Set(uniqueNstList.map(s => s.name.toLowerCase().replace(/\s+/g, ' ')));

const generalPublicPaidGateway = [];
for (const gw of gatewayParsed) {
  const email = gw['Email ID'].toLowerCase().trim();
  const name = gw['Buyer Name'].trim();
  ...
  if (!isAdypu && !allNstEmailSet.has(email) && !allNstNameSet.has(nameNorm)) {
    const totalAmt = parseFloat(gw['Total Amount Transferred']) || 0;
...
console.log(`General Public Paid Gateway Transactions: ${generalPublicPaidGateway.length}`);

// 3. Clear existing NST and insert fresh unified NST records
console.log('\nUpdating Supabase database...');
await supabase.from('registrations').delete().ilike('category', '%NST%');
const { error: insertNstErr } = await supabase.from('registrations').insert(nstRecordsToDb);
if (insertNstErr) console.error('Error inserting NST:', insertNstErr);
else console.log(`✅ Successfully inserted ${nstRecordsToDb.length} NST Students (Paid: ${nstPaidCount}, Pending: ${nstPendingCount}, Revenue: ₹${nstRevenue.toLocaleString('en-IN')})`);
...
let genPaidUpdated = 0;
for (const gen of generalList) {
  const gwMatch = generalPublicPaidGateway.find(gw =>
  ...
  if (gwMatch) {
    await supabase.from('registrations').update({
      ...
      phone: gwMatch.phone && gwMatch.phone.length >= 10 ? gwMatch.phone : gen.phone,
...
console.log(`✅ General Public Registrations reconciled: ${genPaidUpdated} / ${generalPublicPaidGateway.length} matched from web, total ${generalList.length} general runners.`);
