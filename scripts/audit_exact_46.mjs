...
console.log('Total NST form rows parsed:', nstRows.length);
...
for (let i = 0; i < nstRows.length; i++) {
  const n = nstRows[i];
  const found = dbRecords.find(d => 
    d.email.toLowerCase() === n.email ||
    d.email.toLowerCase() === n.email.replace('@email.com', '@gmail.com') ||
    `${d.first_name} ${d.last_name || ''}`.toLowerCase().replace(/[^a-z]/g, '') === n.name.toLowerCase().replace(/[^a-z]/g, '')
  );
  if (found) {
    console.log(`[#${i+1}] MATCHED: ${n.name} (${n.email}) -> DB ID: ${found.id}, DB Category: ${found.category}, emergency_name: ${found.emergency_name}`);
  } else {
    console.log(`[#${i+1}] NOT IN DB: ${n.name} (${n.email})`);
  }
}
