...
async function run() {
  ...
  console.log(`DB records: ${dbRecords.length}, Payments: ${payments.length}`);

  // Map existing chest numbers
  const usedChestNumbers = new Set(
    dbRecords.map(r => parseInt(r.chest_number, 10)).filter(n => !isNaN(n))
  );

  let nextChestNum = 100;
  function getNextAvailableChestNumber() {
    while (usedChestNumbers.has(nextChestNum)) {
      nextChestNum++;
    }
    usedChestNumbers.add(nextChestNum);
    return nextChestNum.toString();
  }
  ...
  // 1. Update existing registrations
  let updatedCount = 0;
  for (const rec of dbRecords) {
    ...
    const { error } = await supabase.from('registrations').update(patch).eq('id', rec.id);
    if (!error) updatedCount++;
  }

  // 2. Insert missing buyers with guaranteed unique chest numbers
  ...
  for (const p of missingFromDb) {
    ...
    const raceType = p.category.includes('Competitive') && !p.category.includes('Non-Competitive')
      ? 'Competitive 5K'
      : 'Non-Competitive Joy 5K';
    const chest = getNextAvailableChestNumber();
    ...
    if (!error) {
      insertedCount++;
      dbPhones.add(p.phone);
      dbEmails.add(p.email);
    } else {
      console.error(`Insert failed for ${p.buyerName}:`, error.message);
    }
  }

  console.log(`\nFinal Sync Complete:`);
  console.log(`- Updated: ${updatedCount}`);
  console.log(`- Inserted new buyers: ${insertedCount}`);
  
  const { data: finalRecords } = await supabase.from('registrations').select('*');
  const paidCount = finalRecords.filter(r => r.payment_status === 'paid').length;
  console.log(`- Total Registrations in Supabase: ${finalRecords.length}`);
