...
const nstRows = parseCSV(nstFormCSV).filter(r => r[1] && !r[1].includes('I understand')).map((r, idx) => ({
  index: idx + 1,
  timestamp: r[0],
  name: r[1].trim(),
...
const inserts = nstRows.map((nst, idx) => {
  let match = gwRows.find(g => {
    if (nst.email && g.email === nst.email) return true;
    if (g.buyerName.toLowerCase() === nst.name.toLowerCase()) return true;
    const nParts = nst.name.toLowerCase().split(/\s+/);
    const gParts = g.buyerName.toLowerCase().split(/\s+/);
    if (nParts.length >= 2 && gParts.length >= 2 && nParts[0] === gParts[0] && nParts[1] === gParts[1]) return true;
    return false;
  });

  const price = match ? match.price : 149;
  const raceType = price === 99 ? 'Non-Competitive Joy 5K' : 'Competitive 5K';
  const customerId = match ? match.customerId : 'unknown';
  const txnId = match ? match.txnId : 'unknown';
  const phone = match && match.phone ? match.phone : '9876543210';

  const parts = nst.name.split(/\s+/);
  const firstName = parts[0] || 'Student';
  const lastName = parts.slice(1).join(' ') || '';

  const bibNumber = `M4S-NST-${101 + idx}`;
  const chestNumber = `NST-${101 + idx}`;

  const cityInfo = `NST ADYPU • ${nst.studyYear || '1st'} Year${nst.urn ? ` • URN: ${nst.urn}` : ''}`;
  ...
  return {
    ...
    blood_group: 'O+',
    dob: `${nst.studyYear || '1st'} Year`,
    weight: nst.weight,
    height: nst.height,
    t_shirt_size: nst.tshirt || 'M',
    email: nst.email || `nst_student_${idx + 1}@adypu.edu.in`,
    phone: phone,
    city: cityInfo,
    emergency_name: nst.driveLink, // Always store Google Drive link
...
const { data: inserted, error: insertError } = await supabase.from('registrations').insert(inserts).select();

if (insertError) {
  console.error('Insert error:', insertError);
} else {
  console.log(`Successfully inserted ${inserted.length} confirmed NST students!`);
}

// Verify database count
const { data: totalNow } = await supabase.from('registrations').select('*');
const nstNow = totalNow.filter(r => (r.category || '').toLowerCase().includes('nst'));
const genNow = totalNow.filter(r => !(r.category || '').toLowerCase().includes('nst'));
console.log(`Total records in DB now: ${totalNow.length} (${nstNow.length} NST Students, ${genNow.length} General Public)`);
