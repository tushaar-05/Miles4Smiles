...
console.log(`Parsed ${gRows.length} gateway transactions and ${nstRows.length} NST form entries.`);

// Match NST entries with Gateway transactions
for (const nst of nstRows) {
  // Try match by email (exact or fixing typos like @email.com -> @gmail.com)
  const normEmail = nst.email.replace('@email.com', '@gmail.com');
  const normName = nst.name.toLowerCase().replace(/[^a-z]/g, '');

  const match = gRows.find(g => 
    g.email === nst.email || 
    g.email === normEmail ||
...
console.log(`Matched ${nstRows.filter(n => n.payment).length} of ${nstRows.length} NST students with official payment gateway.`);
