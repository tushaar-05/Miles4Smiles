...
const nstCSV = `Timestamp,Name,Email,URN NUMBER,Gender,Select your preferred t-shirt size for the event:,Weight,Height,Study Year,"Upload the Screenshot of payment
...
function parseNST(csv) {
  ...
  for (let i = 1; i < lines.length; i++) {
    ...
    if (matches.length >= 9) {
      results.push({
        name: matches[1].trim(),
        email: matches[2].trim().toLowerCase(),
        urn: matches[3].trim().toUpperCase(),
        gender: matches[4].trim(),
        tshirt: matches[5].trim(),
        weight: matches[6].trim(),
        height: matches[7].trim(),
        studyYear: matches[8].trim(),
      });
    }
  }
  return results;
...
async function run() {
  const nstList = parseNST(nstCSV);
  console.log(`Parsed ${nstList.length} NST student records.`);
  ...
  console.log(`Current DB records: ${dbRecords.length}`);

  // Create lookup maps
  const nstByEmail = new Map();
  const nstByName = new Map();

  for (const n of nstList) {
    if (n.email) nstByEmail.set(n.email, n);
    const normName = n.name.toLowerCase().replace(/[^a-z]/g, '');
    if (normName) nstByName.set(normName, n);
  }
  ...
  for (const rec of dbRecords) {
    ...
    const fullName = `${rec.first_name || ''} ${rec.last_name || ''}`.trim().toLowerCase().replace(/[^a-z]/g, '');
    const isAdypuDomain = cleanEmail.includes('@adypu.edu.in');

    const nstInfo = nstByEmail.get(cleanEmail) || nstByName.get(fullName) || (isAdypuDomain ? { urn: cleanEmail.split('@')[0].toUpperCase(), studyYear: '1st' } : null);

    if (nstInfo) {
      taggedCount++;
      const urnStr = nstInfo.urn ? ` (URN: ${nstInfo.urn})` : '';
      const yearStr = nstInfo.studyYear ? ` [${nstInfo.studyYear} Yr]` : '';

      // Set category to 'NST Student'
      // Set city to include URN / Year if city was generic
      const updatedCity = rec.city && rec.city !== 'Pune' ? rec.city : `NST Campus${urnStr}${yearStr}`;
      ...
      if (updErr) console.error(`Error tagging ${rec.first_name}:`, updErr.message);
      else console.log(`✓ Tagged as NST Student: ${rec.first_name} ${rec.last_name} | Email: ${rec.email} | URN: ${nstInfo.urn || 'N/A'}`);
    } else {
      untaggedCount++;
      // Set to 'General Public'
      await supabase
        .from('registrations')
        .update({
  ...
  console.log(`\nTagging Summary:`);
  console.log(`- 🎓 NST Students & Faculty Tagged: ${taggedCount}`);
  console.log(`- 🌍 General Public Runners: ${untaggedCount}`);
}
