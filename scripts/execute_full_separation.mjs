...
02-Sep-2026 00:16,8R6IOZ71,3KH2IIM7,"Sept. 3, 2026, 12:15 a.m.",Gautam Bobade,gautambobade7@gmail.com,8962577474,Miles for Smiles - 5K Charity Run,"Sep 05, 2026 - Sep 05, 2026",1,Competitive 5K,249.00,249.00
...
const gRows = parseCSV(gatewayCSV).map(r => ({
  date: r[0],
  customerId: r[1],
  transactionId: r[2],
  txDateTime: r[3],
...
const nstRows = parseCSV(nstFormCSV).filter(r => r[1] && !r[1].includes('I understand')).map(r => ({
  timestamp: r[0],
  name: r[1].trim(),
  email: (r[2] || '').trim().toLowerCase(),
  urn: (r[3] || '').trim().toUpperCase(),
  gender: r[4].trim(),
  tshirt: r[5].trim().toUpperCase(),
  weight: r[6].trim(),
  height: r[7].trim(),
  studyYear: r[8].trim(),
  driveLink: r[9].trim(),
...
async function execute() {
  ...
  console.log(`Initial DB count: ${dbRecords.length}`);

  // Match and update each NST Student
  let nstBibCounter = 101;
  let genBibCounter = 101;

  for (let i = 0; i < nstRows.length; i++) {
    const nst = nstRows[i];
    const normEmail = nst.email.replace('@email.com', '@gmail.com');
    const normName = nst.name.toLowerCase().replace(/[^a-z]/g, '');

    const gwMatch = gRows.find(g => 
      g.email === nst.email || 
      g.email === normEmail ||
      g.buyerName.toLowerCase().replace(/[^a-z]/g, '') === normName
    );

    const bibNum = `M4S-NST-${nstBibCounter}`;
    const chestNum = `${nstBibCounter}`;
    nstBibCounter++;

    // Find if already exists in Supabase
    const existing = dbRecords.find(r => 
      (r.email && (r.email.toLowerCase() === nst.email || r.email.toLowerCase() === normEmail)) ||
      (r.first_name && `${r.first_name} ${r.last_name || ''}`.toLowerCase().replace(/[^a-z]/g, '') === normName)
    );

    const [firstName, ...rest] = nst.name.split(' ');
    const lastName = rest.join(' ') || '';

    const payload = {
      first_name: firstName,
      last_name: lastName,
      gender: nst.gender || 'Male',
      t_shirt_size: nst.tshirt || 'M',
      weight: nst.weight || '',
      height: nst.height || '',
      email: nst.email,
      phone: gwMatch?.phone || existing?.phone || '9999999999',
      city: `NST Campus [${nst.studyYear || '1st'} Yr] URN: ${nst.urn || 'Pending'}`,
      emergency_name: nst.driveLink || '',
      category: 'NST Student',
      race_type: gwMatch?.raceType || (Number(gwMatch?.amount) === 99 ? 'Non-Competitive Joy 5K' : 'Competitive 5K'),
      amount: gwMatch?.amount || 149.00,
      payment_status: 'paid',
      razorpay_order_id: gwMatch?.customerId || 'NST_DIRECT',
      razorpay_payment_id: gwMatch?.transactionId || 'NST_PAID',
      bib_number: bibNum,
      chest_number: chestNum,
    };

    if (existing) {
      await supabase.from('registrations').update(payload).eq('id', existing.id);
      console.log(`Updated NST: ${nst.name} -> ${bibNum}`);
    } else {
      await supabase.from('registrations').insert([payload]);
      console.log(`Inserted NST: ${nst.name} -> ${bibNum}`);
    }
  }

  // Now assign clean General series for non-NST participants
  const { data: updatedDB } = await supabase.from('registrations').select('*');
  const generalRunners = updatedDB.filter(r => r.category !== 'NST Student');
  
  for (const gen of generalRunners) {
    const bibNum = `M4S-GEN-${genBibCounter}`;
    const chestNum = `${genBibCounter}`;
    genBibCounter++;
  ...
  console.log(`\nSeparation complete:`);
  console.log(`- NST Students: ${nstRows.length} (BIBs M4S-NST-101 to M4S-NST-${nstBibCounter - 1})`);
  console.log(`- General Public: ${generalRunners.length} (BIBs M4S-GEN-101 to M4S-GEN-${genBibCounter - 1})`);
}
