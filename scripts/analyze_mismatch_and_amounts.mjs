...
console.log(`Loaded ${gwRows.length} gateway records.`);

// Let's see how each of the 46 NST students matches with DB records and Gateway records
let matchedInDB = 0;
let unmatchedInDB = [];

for (const nst of nstRows) {
  // Find in DB
  const inDB = dbRecords.find(db => {
    const dbEmail = (db.email || '').toLowerCase();
    const dbName = `${db.first_name} ${db.last_name}`.toLowerCase();
    const nstName = nst.name.toLowerCase();
    const dbCity = (db.city || '').toLowerCase();
    const nstUrn = nst.urn.toLowerCase();

    if (nst.email && dbEmail === nst.email) return true;
    if (nstUrn && nstUrn.length > 3 && dbCity.includes(nstUrn)) return true;
    if (dbName === nstName) return true;
    return false;
  });

  // Find in Gateway to check amount (149 vs 99)
  const inGW = gwRows.find(gw => {
    if (nst.email && gw.email === nst.email) return true;
    if (gw.name.toLowerCase() === nst.name.toLowerCase()) return true;
    return false;
  });
  ...
  if (inDB) {
    ...
    unmatchedInDB.push(nst);
  }
}

console.log(`Matched in DB: ${matchedInDB} / 46`);
console.log('Unmatched in DB:', unmatchedInDB);
