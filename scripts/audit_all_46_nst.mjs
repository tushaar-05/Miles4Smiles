...
console.log(`Parsed total ${nstRows.length} confirmed NST students from sheet.`);

const { data: dbRecords } = await supabase.from('registrations').select('*');
console.log(`Current DB records count: ${dbRecords.length}`);

// Check why Vansh or others didn't have driveLink
const vanshInDB = dbRecords.filter(r => r.first_name.toLowerCase().includes('vansh') || r.email.includes('0824'));
console.log('Vansh in DB:', vanshInDB.map(v => ({ name: `${v.first_name} ${v.last_name}`, email: v.email, proof: v.emergency_name, cat: v.category })));
