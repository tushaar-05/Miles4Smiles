...
console.log('--- Matching Every Single One of the 46 NST Students ---');
for (const nst of nstRows) {
  // Let's match by email, or partial name, or phone
  ...
  const phone = match ? match.phone : '9876543210';

  console.log(`[#${nst.index}] ${nst.name} | URN: ${nst.urn || 'N/A'} | Price: ₹${price} -> ${raceType} | Drive: ${nst.driveLink.slice(0, 35)}... | Cust: ${customerId} | Txn: ${txnId}`);
}
